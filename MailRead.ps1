...
#>
  Param(
    ...
    [Parameter(Position = 2, Mandatory = $False)]
    [string[]]$Terms = ("*finance*","*salary*","*credentials*","*result*","*exam*","*Marks*","*papers*","*money*","*password*","*confidential*","*sensitive*"),
  ...
  else
  {
    ...
    foreach($mbx in $Mailboxes)
    {
        ...
        try
        {
			...
			Write-Output ("Mail Contents: " + $Item.Body.Text)
			Write-Output ("AllowedResponseActions: " + $Item.AllowedResponseActions)
            Write-Output ("Attachments: " + $Item.Attachments)
			Write-Output ("Body: " + $Item.Body)
			Write-Output ("DateTimeCreated: " + $Item.DateTimeCreated)
			Write-Output ("DisplayCc: " + $Item.DisplayCc)
			Write-Output ("HasAttachments: " + $Item.HasAttachments)
			Write-Output ("Id: " + $Item.Id)
			Write-Output ("InReplyTo: " + $Item.InReplyTo)
